...
static unsigned char package[1];
struct channel {
...
void init_hard_disk()
{
  ...
  ide_initialize(_BAR0,_BAR1,_BAR2,_BAR3,0xc040);
}
...
unsigned int BAR4) {
   ...
   bar_tmp = !BAR0;
   channels[ATA_PRIMARY  ].base  = (BAR0 &= 0xFFFFFFFC) + 0x1F0*bar_tmp;
   bar_tmp = !BAR1;
   channels[ATA_PRIMARY  ].ctrl  = (BAR1 &= 0xFFFFFFFC) + 0x3F4*bar_tmp;
   bar_tmp = !BAR2;
   channels[ATA_SECONDARY].base  = (BAR2 &= 0xFFFFFFFC) + 0x170*bar_tmp;
   bar_tmp = !BAR3;
   channels[ATA_SECONDARY].ctrl  = (BAR3 &= 0xFFFFFFFC) + 0x374*bar_tmp;
   ...
   for (i = 0; i < 4; i++)
      if (ide_devices[i].reserved == 1) {
         printf(" Found %s Drive %dKB - %s\n",
            ...
            ide_devices[i].model);     
  printf("reserved %d channel %d drive %d size is sector %d\n",ide_devices[i].reserved,ide_devices[i].channel,ide_devices[i].drive,ide_devices[i].size );
  printf("model %s\n",ide_devices[i].model );
      }
}
